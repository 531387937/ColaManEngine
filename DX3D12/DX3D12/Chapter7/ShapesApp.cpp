...
void ShapesApp::BuildConstantsBufferViews()
{
    ...
    int objectCount = mOpaqueRitems.size();
    for(int frameIndex = 0;frameIndex<gNumFrameResources;frameIndex++)
    {
        ...
        {
            D3D12_GPU_VIRTUAL_ADDRESS address = objectCB->Resource()->GetGPUVirtualAddress();

            address+= objectByteSize*i;
            ...
            handle.Offset(frameIndex*objectCount+i,mCbvSrvUavDescriptorSize);
    ...
    for(int frameIndex = 0;frameIndex<gNumFrameResources;frameIndex++)
    {
        ...
        D3D12_GPU_VIRTUAL_ADDRESS address = passCB->Resource()->GetGPUVirtualAddress();
        ...
        auto handle = CD3DX12_CPU_DESCRIPTOR_HANDLE(mCbvHeap->GetCPUDescriptorHandleForHeapStart());
        handle.Offset(objectCount*3+frameIndex,mCbvSrvUavDescriptorSize);
...
void ShapesApp::BuildRootSignature()
{
   ...
   CD3DX12_ROOT_SIGNATURE_DESC sigDesc(2,slotParameters,0,nullptr,D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
